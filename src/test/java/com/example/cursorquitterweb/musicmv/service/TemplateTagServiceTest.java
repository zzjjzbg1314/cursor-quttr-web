...
class TemplateTagServiceTest {
    ...
    @Test void SQLite验证新增事务和生日迁移不会重复补回() throws Exception {
        ...
        String script=String.join("\n",
            ...
            "expected=['birthday','wedding','valentines-day','mothers-day','christmas','graduation','fathers-day']",
            "assert [r[0] for r in db.execute('SELECT tag_key FROM template_tags ORDER BY sort_order,tag_key')]==expected",
            "db.execute('UPDATE template_tags SET sort_order=100-sort_order')",
            "db.execute(\"DELETE FROM template_tag_migrations WHERE migration_key='occasion-priority-v1'\");db.commit();run(batches[0])",
            "assert [r[0] for r in db.execute('SELECT tag_key FROM template_tags ORDER BY sort_order,tag_key')]==expected",
            "db.execute(\"UPDATE template_tags SET sort_order=99 WHERE tag_key='birthday'\");db.commit();run(batches[0])",
            "assert db.execute(\"SELECT sort_order FROM template_tags WHERE tag_key='birthday'\").fetchone()[0]==99",
