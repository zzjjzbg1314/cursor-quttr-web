...
/**
 * Google JWT Token 验证工具类
 */
public class GoogleJwtUtil {
    
    private static final Logger logger = LogUtil.getLogger(GoogleJwtUtil.class);
     ...
     * @param expectedAudience 期望的 audience（通常是你的 Google Client ID）
     * @return 如果验证成功返回 token 中的 sub（Google User ID），否则返回 null
     */
    public static String verifyIdToken(String idToken, String expectedAudience) {
        try {
            // 1. 分割 JWT token
            ...
            if (expectedAudience != null && !expectedAudience.isEmpty() && !aud.equals(expectedAudience)) {
                logger.error("Audience 不匹配，期望: {}, 实际: {}", expectedAudience, aud);
                return null;
            }
            ...
            if (publicKey == null) {
                logger.error("无法获取 Google 公钥，kid: {}", kid);
                return null;
            }
            ...
            if (verified) {
                logger.info("Google ID Token 验证成功，sub: {}", sub);
                return sub;
            } else {
                logger.error("签名验证失败");
                return null;
            ...
            logger.error("验证 Google ID Token 失败", e);
            return null;
        }
    }
     ...
     * 获取 Google 公钥
     */
    private static PublicKey getGooglePublicKey(String kid) {
        try {
            // 检查缓存
     ...
     * 仅用于调试目的
     */
    public static JsonNode decodePayload(String idToken) {
        try {
            String[] parts = idToken.split("\\.");
            if (parts.length != 3) {
                return null;
