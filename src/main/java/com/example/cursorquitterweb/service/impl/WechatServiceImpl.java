...
public class WechatServiceImpl implements WechatService {
    ...
    public WechatUserInfo login(String code) {
        ...
        try {
            ...
            if (sessionInfo == null) {
                LogUtil.logError(logger, "获取微信session失败");
    ...
    private Map<String, String> getSessionInfo(String code) {
        try {
            ...
            LogUtil.logDebug(logger, "请求微信session URL: {}", url);
            
            ResponseEntity<String> response = restTemplate.getForEntity(url, String.class);
            String responseBody = response.getBody();
            
            LogUtil.logDebug(logger, "微信session响应: {}", responseBody);
            
            JsonNode jsonNode = objectMapper.readTree(responseBody);
            
            if (jsonNode.has("errcode") && jsonNode.get("errcode").asInt() != 0) {
                String errorMsg = jsonNode.has("errmsg") ? jsonNode.get("errmsg").asText() : "未知错误";
                LogUtil.logError(logger, "微信session获取失败: {}", errorMsg);
                return null;
            }
            ...
            LogUtil.logError(logger, "获取微信session失败", e);
            return null;
        }
    }
} 