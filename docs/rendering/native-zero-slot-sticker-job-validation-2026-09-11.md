...
代码提交 dc3f10a 后，将已通过测试的 5 个服务 class 文件校验一致后接入 target/classes，精确停止原 8080 网站进程并沿用原配置启动，未输出凭据或修改配置。新 PID=21922，健康检查通过；启动前后公开目录的 8 个 templateId/currentVersionId 集合一致。该接入检查证明运行服务加载本次代码，不等于实际创建纯贴纸云端模板。原进程 class 备份及私有启动日志位于 `/tmp/native-sticker-slot-fix/rollout/`。
